...
<div id="tt" class="easyui-tabs" style="width: 100%;">
	<div title="资产报废查询" style="padding: 20px; display: none;">
		<table id="jyss" border="1" bordercolor="#8DB2E3" title="信息查询" style="border-collapse: collapse; width: 100%;">
			...
			<tr>
				...
				<td><select class="easyui-combobox" id="ckmc" name="ckmc" style="width: 150px; height: 20px;"></select></td>
			</tr>
		</table>
...
<script type="text/javascript">
	...
	function search() {
		...
		var ckmc= $("#ckmc").combobox('getValue');
	...
	function selectFun(id){
		 ...
		 $.ajax({
	            url: "${ctx}/zcbf/queryShow.do?crkid="+id,
	            type: "POST",
	...
	function selectSb(value) {
		if(value!='' && value!='undefined') {
			...
			$("#sbid").combobox({
				 url:'${ctx}/sbb/selectSbCombobox.do?wply=1&lx=1&dqzt=7',
				 valueField:'sbid',
				 textField:'pm',
				 editable:false,
				 onChange:function(nv,ov) {
	...
	function addFun() {
		...
		//document.getElementById("#ztce").style.display='';
		//$('#ztce').attr('type','hidden');
		//隐藏
		$('#zt').next(".combo").hide();
		clearWpDw();
		$('#dlg').dialog('open');
	...
	/*删除+ '&sid=' + sid*/
	function deleteFun(id,sid) {
		parent.$.messager.confirm('询问', '您是否要删除当前数据？',
				function(b) {
					if (b) {
						$.post(
							'${ctx}/zcbf/delete.do?crkid=' + id , 
							function(result) {
	...
	function editFun(id) {
		 $.ajax({
	            url: "${ctx}/zcbf/editShow.do?crkid="+id,
	            type: "POST",
	            contentType: "application/json;charset=utf-8",
	            dataType: "json",
	            success: function (data) {
	            	if(data.success == true) {
	            		 // document.getElementById("#ztce").style.display=none;
	            		$('#ztce').attr('type','hidden');
	            		$('#zt').next(".combo").show();
	            		var result = data.t;
	            		if(null != result.crkid) {
...
<!-- 添加编辑模态框 -->
<div id="dlg" class="easyui-dialog" style="width: 400px; height: 350px;display:none;" closed="true" buttons="#dlg-buttons" title="资产报废">
	<form action="" id="dlgForm">
		<table style="margin:auto">
			...
			<tr>
				<td>物品数量:</td>
				<td><input readonly="readonly" style="border: 0px;outline:none;" type="text" id="sl" value=""  /></td>
			</tr>
			 <tr>
				<td>类型</td>
				<td><input readonly="readonly" style="border: 0px;outline:none;" type="text" id="lx" value=""  /></td>
			...
			<tr>
				<td>报废数量</td>
				<td>
					<input data-options="required:true" id="lysl" name="lysl" class="easyui-numberspinner" style="width: 150px; height: 20px;"/>
...
<!-- 查看页面 -->
<div id="dlgshow" class="easyui-dialog" style="width: 400px; height: 470px;display:none;" closed="true" buttons="#dlgshow-buttons" title="资产报废查看">
	<form action="" id="dlgFormshow">
		<table style="margin:auto">
			...
			<tr>
				<td>物品规格:</td>
				<td><input style="width: 150px; height: 20px;" readonly="readonly"  type="text" id="show_zcgg" value="" class="easyui-textbox" /></td>
			</tr>
			<tr>
				<td>一级分类:</td>
			...
			<tr>
				<td>货架</td>
				<td>
					<input style="width: 150px; height: 20px;" readonly="readonly"  type="text" id="show_hj" value="" class="easyui-textbox" />
				</td>
			</tr>
			<tr>
				<td>报废数量</td>
